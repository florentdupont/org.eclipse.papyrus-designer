 ...
 *
 */
public class DropReverseStrategy2 extends TransactionalDropStrategy {
	public DropReverseStrategy2() {
	}
...
//					System.err.println("Can't compute relatif model uid. Use absolute one");
	...
	protected Command doGetCommand(Request request, EditPart targetEditPart) {
		...
		ReverseCommandParameters cmdParameters = new ReverseCommandParameters(gtEditPart, targetView, location);
		
		org.eclipse.emf.common.command.Command reverseCmd = getReverseCommand(editingDomain, getCurrentSelection(), umlModel, cmdParameters);
		// Checks if the command can be executed
		if( !reverseCmd.canExecute() ) {
			return null;
		}
				
		cc.add( new EMFtoGEFCommandWrapper( reverseCmd ) );
		CreateNodesAtLocationCommand createCmd = new CreateNodesAtLocationCommand(editingDomain, "", targetView, cmdParameters);
