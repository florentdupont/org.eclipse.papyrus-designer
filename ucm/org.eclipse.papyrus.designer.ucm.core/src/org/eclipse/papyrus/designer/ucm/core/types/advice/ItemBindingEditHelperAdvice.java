...
/**
 * Edit Helper Advice for {@link ItemBinding}
 */
public class ItemBindingEditHelperAdvice extends AbstractEditHelperAdvice {
	...
	/**
	 * Check the creation condition for the creation of an item binding
	 *
	 * @param request
	 *            the create request
	...
	protected boolean approveCreateElementRequest(CreateElementRequest request) {
		...
		if (type != null && container instanceof StructuredClassifier) {
			if (UMLUtil.getStereotypeApplication((StructuredClassifier) container, ConnectorDefinition.class) != null) {
				return true;
			}
		}
		return false;
